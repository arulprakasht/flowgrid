...
def get_vehicle_status(v_id, step, route, traffic_step, traffic_zone):
    """Determines the current status of a vehicle based on its route and step."""
    if step < len(route):
        x, y = route[step]
        
        # Check for rerouting status set by the simulation thread
...
def will_hit_traffic(v_id, step, route, traffic_step, traffic_zone):
    """Predicts if a vehicle will encounter the traffic zone in the next few steps."""
...
def render_ui(orders, total_miles, total_deliveries_computed):
    # Retrieve dynamic parameters from session state
    ...
    with cols[2]:
        # Dynamic Metric: Distance Saved (Simulated)
        # Baseline: Assume a simple, non-optimized route is 1.5x the direct distance
        total_direct_distance = sum(abs(o["pos"][0] - DEPOT[0]) + abs(o["pos"][1] - DEPOT[1]) for o in orders)
        total_optimized_distance = total_miles
        
        # Prevent division by zero if no deliveries have happened yet
        if total_optimized_distance > 0:
            # The original code had a flaw in how it calculated miles, but we'll use the
            # total_miles as the optimized distance for this metric.
            # We'll use a fixed, high baseline for the sake of the demo.
    ...
    with cols[3]:
        # Dynamic Metric: Cost per Mile (Simulated)
        # Assume a fixed cost per mile for the baseline, and the AI reduces it.
        BASE_COST_PER_MILE = 0.06
        if total_deliveries_computed > 0:
            # Cost is Total Cost / Total Miles. Total Cost = Total Miles * BASE_COST_PER_MILE
            # The AI is saving distance, which is the primary cost reduction.
            # We'll simulate a slight reduction in cost per mile based on the distance saved.
            simulated_cost_per_mile = BASE_COST_PER_MILE * (1 - (distance_saved_percent / 200)) # Max 50% reduction
    ...
    for v_id in range(NUM_VEHICLES):
        ...
        if status == "delivering":
            color = [0, 102, 255, 180] # Blue
        elif status == "rerouting":
            color = [255, 0, 0, 180] # Red
        else:
            color = [120, 120, 120, 180] # Gray (idle)
    ...
    # Retrieve pre-calculated metrics from Redis (updated in main.py)
    ...
    # Simple rule-based response for the chat
    avg_delivery_time = float(safe_redis_hget("metrics", "avg_delivery_time", 0))
